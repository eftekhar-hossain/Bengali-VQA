...
class BVQADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        im_inputs = self.image_transform(image)
...
val_dataset = BVQADataset(dataframe = valid, tokenizer = tokenizer,data_dir = image_dir,
                            max_seq_length=15)
val_loader = DataLoader(val_dataset, batch_size=16,shuffle=False)

# test dataloader
test_dataset = BVQADataset(dataframe = test, tokenizer = tokenizer,data_dir = image_dir,
                            max_seq_length=15)
...
class MCRAN(nn.Module):
    def __init__(self, num_classes,n_layers,num_heads, model_name):
        ...
        if model_name == 'resnet':
            ...
            self.features = torch.nn.Sequential(*list(self.imagenet.children())[:-2])  
            self.conv1d = nn.Conv1d(in_channels=2048, out_channels=768, kernel_size=1)
        elif  model_name == 'convex': 
            #ConVex Net
            self.imagenet = models.convnext_base(pretrained=True)
            # Exclude the last FC layer
            self.features = torch.nn.Sequential(*list(self.imagenet.children())[:-2])  
            ...
            #ConVex Net
            self.imagenet = models.efficientnet_b0(pretrained=True)
            # Exclude the last FC layer
            self.features = torch.nn.Sequential(*list(self.imagenet.children())[:-2])  
            self.conv1d = nn.Conv1d(in_channels=1280, out_channels=768, kernel_size=1) 
    ...
    def forward(self, image_input, input_ids, attention_mask):
        ...
        # Extract visual features using ResNet152
        image_features = self.features(image_input)  #[batch_size, 2048, 7, 7]
        image_features = image_features .flatten(start_dim=2)
        #print(image_features.shape)
        # change 2048 channels to 768
        image_features = self.conv1d(image_features)
        #print(image_features.shape)
        ...
        image_features_new = F.adaptive_avg_pool1d(image_features.permute(0, 2, 1), 15).permute(0, 2, 1)
        #print(image_features_new.shape)
...
# num_heads = 6  # Number of attention heads for multihead attention
...
def main(args):
    ...
    model = MCRAN(num_classes, n_layers, num_heads, args.model_name)
    model = model.to(device)

    print("Image model name: ",args.model_name)
    ...
    for epoch in range(num_epochs):
        ...
        if val_accuracy > best_val_accuracy:
            ...
            torch.save(model.state_dict(), os.path.join(model_dir, f'{args.model_name}.pth'))
            print("Model Saved.")
        else:
    ...
    model.load_state_dict(torch.load(os.path.join(model_dir,f'{args.model_name}.pth')))
    model.eval()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Varied Image Encoder-VQA')
